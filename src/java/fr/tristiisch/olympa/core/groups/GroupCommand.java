...
public class GroupCommand extends OlympaCommand {
	...
	public GroupCommand(Plugin plugin) {
		super(plugin, "group", "Permet la gestion des groupes de Olympa.", OlympaPermission.GROUP_COMMAND, "groupe", "rank");
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			if (target == null) {
				...
				if (olympaTarget == null) {
					Collection<String> pentialsPlayers = Utils.similarWords(args[0], MySQL.getAllPlayersNames());
		...
		if (args.length <= 1) {
			...
			if (newGroup == null) {
				Collection<String> pentialsGroup = Utils.similarWords(args[1], Arrays.stream(OlympaGroup.values()).map(OlympaGroup::getName).collect(Collectors.toSet()));
