	...
	public void onCommand(CommandSender sender, String[] args) {
		...
		servers.entrySet().stream().sorted((o1, o2) -> Integer.compare(o2.getValue().size(), o1.getValue().size())).forEach(entry -> {
			...
			for (int i = 0; i < players.size(); i++) {
				...
				}else playerC = new TxtComponentBuilder("%s%s", oplayer.getGroup().getColor(), oplayer.getName()).onHoverText(oplayer.getGroupNameColored());
				if (i + 1 < players.size()) playerC.extra(",");
				out2.extra(playerC);
			}
