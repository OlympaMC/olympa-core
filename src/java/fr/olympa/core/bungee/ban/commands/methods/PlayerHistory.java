...
public class PlayerHistory {
	...
	public static void histBan(CommandSender sender, String name, UUID uuid) {
		...
		if (bans.size() == 0) {
			sender.sendMessage(SpigotUtils.color(Prefix.DEFAULT_BAD + "&4%player%&c n'a jamais été sanctionné.").replaceAll("%player%", olympaTarget.getName()));
			return;
		}

		TextComponent msg = new TextComponent(SpigotUtils.color("&6Historique des sanctions de " + olympaTarget.getGroup().getPrefix() + olympaTarget.getName() + "&6:\n"));
