...
public class ForceKickCommand extends BungeeCommand {
	...
	public void onCommand(CommandSender sender, String[] args) {
		...
		if (Matcher.isUsername(args[0])) {
			...
			if (target == null) {
				this.sendMessage(ColorUtils.color(Prefix.DEFAULT_BAD + "Le joueur &4" + args[0] + "&c n'est pas connecté."));
				return;
			}
			target.getPendingConnection().disconnect();
			this.sendMessage(ColorUtils.color(Prefix.DEFAULT_GOOD + "Le joueur &2" + target.getName() + "&a a été kick."));
