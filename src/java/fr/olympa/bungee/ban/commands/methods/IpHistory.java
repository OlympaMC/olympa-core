...
public class IpHistory {
	...
	public static void histBan(CommandSender sender, String ip) {
		...
		List<EmeraldPlayer> players = MySQL.getPlayersByIp(ip);

		String playersShow = players.stream().map(EmeraldPlayer::getName).collect(Collectors.joining(", "));
		...
		TextComponent msg = new TextComponent(Utils.color("&6Historique des sanctions de l'ip de &e" + playersShow + "&6:\n"));
